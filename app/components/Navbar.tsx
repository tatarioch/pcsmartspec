      ...
      >
        ...
        <div className="mx-auto w-full flex items-center justify-between px-4 py-2 h-16 ">
          <div className="flex items-center gap-3">
            <img
              src="/logo.png"
              alt="Royal Smart Computer Logo"
              className="h-10 w-10 rounded-lg object-contain shadow-md transition-transform duration-300 hover:scale-105"
            />
          ...
          <div className="md:hidden flex items-center">
            ...
            >
              <i
                className={`fa-solid fa-bars text-lg transition-transform duration-300 ${isOpen ? "rotate-90 scale-110" : "rotate-0"
                  }`}
              />
            </button>
          </div>
          ...
          <div className="flex flex-col gap-1 px-0 py-3">
            {menuItems.map((item, index) => (
              <Link
                key={item.href}
                href={item.href}
                className={`inline-flex items-center gap-3 px-4 py-3 transition-all duration-500 transform w-full ${isOpen ? "translate-x-0 opacity-100" : "translate-x-4 opacity-0"
                  } ${isActive(item.href)}`}
            ...
            <button
              className={`inline-flex items-center gap-3 px-4 py-3 w-full bg-red-50 text-red-600 hover:bg-red-100 transition-all duration-500 transform ${isOpen ? "translate-x-0 opacity-100" : "translate-x-4 opacity-0"
                }`}
              style={{
